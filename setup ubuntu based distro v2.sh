...
sudo apt purge firefox* thunderbird* geary* hexchat* -y 
...
sudo sh -c 'echo "deb [arch=amd64] http://dl.google.com/linux/chrome/deb/ stable main" >> /etc/apt/sources.list.d/google.list' && wget -q -O - https://dl.google.com/linux/linux_signing_key.pub | sudo apt-key add - && sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 379CE192D401AB61 
...
sudo apt install -y google-chrome-stable darktable gimp inkscape krita photocollage rapid-photo-downloader scribus shotwell ufraw rawtherapee gnome-tweak-tool 
...
sudo apt install -y samba system-config-samba ubuntu-restricted-extras mp3gain snapd apt-xapian-index gnome-tweak-tool flatpak 
sudo apt install -y libreoffice-writer libreoffice-impress libreoffice-calc libreoffice-draw ibus-libpinyin ibus ibus-pinyin
